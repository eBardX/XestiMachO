...
public extension MachObject {
    ...
    func dump(_ arch: FatArch) {
        print("architecture \(arch.header.architecture)")
        print("    cputype \(arch.header.cpuType)")
        print("    cpusubtype \(arch.header.cpuSubtype)")
        print("    capabilities 0x0")
        print("    offset \(arch.header.offset)")
    ...
    func dump(_ header: FatHeader) {
        ...
        print("nfat_arch \(header.archs.count)")

        for arch in header.archs {
            dump(arch)
        }
    ...
    func dump(_ header: MachHeader) {
        ...
        for (index, command) in header.commands.enumerated() {
            dump(command, index)
        }
    }
